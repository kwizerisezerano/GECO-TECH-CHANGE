...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $pdo->prepare("SELECT * FROM partners WHERE partner_id = ?");
    $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE partners SET partner_name = ? WHERE partner_id = ?");
    if ($stmt->execute([$name, $id])) {
        echo '<script>alert("Partner updated successfully!"); window.location.href="view_partners.php";</script>';
    } else {
        echo '<script>alert("Failed to update partner."); window.location.href="update_partner.php?id=' . $id . '";</script>';
    }
}
?>
...
<div class="container">
        ...
        <form method="POST">
            <input type="hidden" name="id" value="<?= htmlspecialchars($partner['partner_id']) ?>">
            ...
            <button type="submit" class="btn btn-upload">Update Partner</button>
