...
if (isset($_POST['add'])) {
    ...
    if (empty($first_name) || empty($last_name) || empty($phone_number) || empty($role)) {
    ...
    try {
        ...
        $sql_check = "SELECT COUNT(*) FROM members WHERE idno = :idno";
        ...
        $sql = "INSERT INTO members (first_name, last_name, phone_number, idno_type, idno, role) 
                VALUES (:first_name, :last_name, :phone_number, :idno_type, :idno, :role)";
        ...
        $stmt->bindParam(':role', $role, PDO::PARAM_STR); // Bind the role

        // Execute statement
        if ($stmt->execute()) {
            echo "<script>
                    alert('Member added successfully.');
                    window.location.href = 'view_members.php'; // Redirect after success
                  </script>";
        } else {
            echo "<script>
